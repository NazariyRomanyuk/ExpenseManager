...
internal static class DummyStorage
{
    ...
    internal static List<WalletDBModel> Wallets => _wallets;
    internal static List<TransactionDBModel> Transactions => _transactions;

    static DummyStorage()
    {
        var uahWallet = new WalletDBModel("Hryvnia Wallet", WalletCurrency.UAH);
        var usdWallet = new WalletDBModel("Dollar Wallet", WalletCurrency.USD);
        var eurWallet = new WalletDBModel("Euro Wallet", WalletCurrency.EUR);
        ...
        _transactions = new List<TransactionDBModel>()
        {
            ...
            new TransactionDBModel(uahWallet.Id, -384.57m, PaymentCategory.OnlineServices, "Google One payment", new DateTime(2025,10,28,20,00,00)),
